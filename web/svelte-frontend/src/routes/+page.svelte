<script lang="ts">
	...
	async function loadPage(p: number, q: string | null = null) {
		...
		try {
			...
			if (q) {
				const res = await fetch(`http://127.0.0.1:8421/api/search?q=${encodeURIComponent(q)}&limit=${LIMIT}&offset=${offset}`);
				if (!res.ok) throw new Error('Search failed');
				const data = await res.json();
				entries = data.entries ?? [];
				total = data.total ?? entries.length;
			} else {
				const res = await fetch(`http://127.0.0.1:8421/api/entries?limit=${LIMIT}&offset=${offset}`);
