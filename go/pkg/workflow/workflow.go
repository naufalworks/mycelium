...
// Workflow defines a complete structured execution plan.
...
// Define registers a new workflow as a prompt in the registry.
func (e *Engine) Define(w *Workflow) error {
	if w.Name == "" || len(w.Steps) == 0 {
		return fmt.Errorf("workflow must have a name and at least one step")
	}

	data, _ := json.Marshal(w)
	now := time.Now().UTC().Format(time.RFC3339)

	// Store as a prompt with full workflow data
	err := e.promptReg.Define(prompts.Prompt{
		Name:        w.Name,
		Template:    string(data),
		OutputShape: `{"type": "object", "properties": {"status": {"type": "string"}, "steps": {"type": "array"}}}`,
...
// Get retrieves a stored workflow definition.
...
// Status returns the current state of a running workflow.
func (e *Engine) Status(runID string) *RunState {
	e.mu.Lock()
	defer e.mu.Unlock()
	return e.runs[runID]
}

// ListRuns returns all active run states.
func (e *Engine) ListRuns() []*RunState {
	e.mu.Lock()
	defer e.mu.Unlock()
	var runs []*RunState
...
// Advance moves the workflow to the next step, storing results.
func (e *Engine) Advance(runID string, stepName string, output string, errMsg string) error {
	e.mu.Lock()
	run, ok := e.runs[runID]
	e.mu.Unlock()
	if !ok {
...
// Log generates an audit trail for a completed workflow run.
